...
export const exportToolDocs: ToolSpec[] = [
  {
    ...
    description:
      "Lists registered export codecs and their current-format compatibility.",
...
function writable(raw: unknown): WritableResult {
  ...
  const data = stringifyUnknown(raw);
  return {
    data,
...
function sha256(data: string | Buffer): string {
  ...
  const cryptoModule = requireNativeModule("crypto", {
    ...
    createHash: (algorithm: string) => {
      update: (value: string | Buffer) => { digest: (encoding: "hex") => string };
    };
  };
...
function rawBounds(): {
  ...
  for (const mesh of Mesh.all) {
    const vertices = (mesh as unknown as {
...
function writeCheckpointPair(
  ...
  for (const file of [modelTemp, metadataTemp, modelBackup, metadataBackup]) {
  ...
  try {
    ...
    if (fs.existsSync(metadataBackup)) {
      fs.rmSync(metadataBackup, { force: true });
    }
  } catch (error) {
    for (const file of [modelPath, metadataPath]) {
      if (fs.existsSync(file)) fs.rmSync(file, { force: true });
    }
    if (fs.existsSync(modelBackup)) fs.renameSync(modelBackup, modelPath);
    if (fs.existsSync(metadataBackup)) {
      fs.renameSync(metadataBackup, metadataPath);
    }
    for (const file of [modelTemp, metadataTemp]) {
      if (fs.existsSync(file)) fs.rmSync(file, { force: true });
    }
    throw error;
  }
...
function referenceManifestHash(
  fs: NativeFsLike,
  sessionRoot?: string
): string | null {
  ...
  createTool(
    exportToolDocs[0].name,
    {
      ...
      async execute({ only_current_format }) {
        ...
        // @ts-ignore - Blockbench runtime global.
        const currentCodec = (Format as { codec?: { id?: string } } | undefined)
          ?.codec?.id;
        const codecs = Object.entries(registry)
          ...
          .filter(
            (codec) => !only_current_format || codec.belongs_to_current_format
          )
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Found ${codecs.length} export codec(s).`,
            },
          ],
          structuredContent: {
            status: "PASS",
            current_format_codec: currentCodec ?? null,
            codecs,
          },
        };
      },
  ...
  createTool(
    exportToolDocs[1].name,
    {
      ...
      async execute({
        ...
        const registry = getCodecRegistry();
        // @ts-ignore - Blockbench runtime global.
        const formatCodec = (Format as { codec?: { id?: string } } | undefined)
          ?.codec;
        ...
        if (!codec || typeof codec.compile !== "function") {
          throw new Error(
            `Codec "${resolvedId}" is unavailable or cannot compile.`
          );
        }
        ...
        if (path) {
          const fs = nativeFs(
            `MCP export_model requested write access to ${path}`
          );
  ...
  createTool(
    exportToolDocs[2].name,
    {
      ...
      async execute({
        ...
        if (!model.byteLength) {
          throw new Error("Project codec returned empty checkpoint output.");
        }

        const fs = nativeFs(
          `MCP save_project_checkpoint requested write access to ${path}`
        );
        ...
        const baseMetadata = {
          ...
          counts: {
            ...
            // @ts-ignore - Blockbench runtime animation registry.
            animations: Animation.all?.length ?? 0,
          },
          raw_model_bounds: rawBounds(),
          accepted_areas,
          open_issues,
          byte_length: model.byteLength,
          encoding: model.encoding,
          bbmodel_sha256: sha256(model.data),
          reference_manifest_sha256: referenceManifestHash(fs, session_root),
        };
