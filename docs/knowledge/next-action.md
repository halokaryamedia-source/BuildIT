...
A bounded GitHub-only animation contract correction now preserves an explicitly accepted `animation_length: 0` when constructing the Bedrock animation payload, rejects empty `manage_keyframes` requests, and requires an explicit `loop_mode` for the timeline loop mutation. AnimationCodec behavior and playback semantics are unchanged.
