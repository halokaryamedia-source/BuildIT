...
Updated: 2026-09-06 — Geometry surface-gap diagnostic landed; targeted live proof is next.
...
1. Surface-gap defect only:
   - `cd mcp`
   - `bun test tests/authoring-quality-diagnostics.test.ts`
   ...
   - On the rejected asset, call `inspect_model_bounds` once plus fresh affected view(s). Expected: `Possible coplanar edge-gap` identifies the Cube pair/distance while open and disappears after closure.
   - Do **not** run full Local Acceptance or `verify:full` just for this defect.
2. When broader Local contract work resumes: `bun install --frozen-lockfile` → close Resource/Prompt/handoff + generated prompt/docs → `bun run verify:full` → deploy/Gateway/DIRECT smoke.
