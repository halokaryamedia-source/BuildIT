...
`create_project` no longer exposes a redundant `format` input. The normal BlockIT project format is a fixed product invariant (`bedrock`), so the removed literal/default field never changed runtime behavior and only added schema/tool-call noise. Project creation still uses native `newProject(Formats.bedrock)` and returns `format.id = bedrock`.
...
Cube authoring inputs now keep identity and UV intent deterministic. Create/rename Cube names cannot be empty. `place_cube` no longer exposes generic per-Cube texture selection, `faces:false`, or face-name-only texture routing because Bedrock Entity is native `single_texture` and those paths either do not own effective texture selection or depend on ambient editor face state. Default UV intent inherits the project mode; explicit non-empty custom face UV rectangles require finite values and make only the new Cube per-face UV (`box_uv=false`). No arbitrary Cube-batch size cap, separate UV framework, or face-disable capability was added.
...
The final GitHub-only pinned-SDK `tools/list` measurement for the current source state is **62 enabled tools / 72,067 response characters**, including **47,966 characters of input schemas** and **11,800 characters of tool descriptions**. `create_project` remains **212** schema characters and `add_group` is **758**; `list_export_formats`, `apply_texture`, and `filter_by_material` are absent from the callable surface while `export_model` remains exposed. Historical wire totals remain source-state evidence, not a controlled performance benchmark. The next large usage lever still requires real Codex native lazy/deferred MCP tool loading proof before any BlockIT router/profile architecture.
