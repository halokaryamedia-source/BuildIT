...
The actual stateless MCP `tools/list` wire surface is currently **63 enabled tools / 73,174 response characters**, including **48,551 characters of input schemas** and **12,108 characters of tool descriptions**. `place_cube` itself is now 2,531 schema characters and no longer exposes a per-Cube `texture` field. Historical 65/64/63-tool byte totals remain source-state evidence, not a controlled performance benchmark. The next large usage lever still requires real Codex native lazy/deferred MCP tool loading proof before any BlockIT router/profile architecture.
