...
The completed runbook is procedural/history evidence and becomes active only when `next-action.md` explicitly points back to local acceptance.

## MCP sources

Start with only the source needed by the current decision:
...
- [Skill Map](../skills/skill-map.md) — inventory/lineage.
...
Historical Git revisions remain available for obsolete implementation/planning detail. Do not keep duplicate active documents solely for provenance.
