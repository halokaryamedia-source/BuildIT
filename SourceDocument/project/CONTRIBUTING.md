...
## Project Structure
- `src/index.ts`: Plugin entry; registers server, UI, settings.
...
- `src/ui/`: Panel, settings, and status bar UI.
- `src/lib/`: Shared utilities, factories (`createTool`, `createResource`, `createPrompt`), and Zod schemas.
- `src/macros/`: Build-time macros (e.g., prompt embedding).
- `dist/`: Build outputs (`mcp.js`, maps, copied assets).

## Adding Tools
Use `createTool()` from `src/lib/factories.ts`. Tools are organized by domain in `src/server/tools/` (e.g., `animation.ts`, `paint.ts`, `mesh.ts`). Each domain file exports a registration function that is called from `src/server/tools.ts`.

Example tool in a domain file (e.g., `src/server/tools/example.ts`):
```ts
import { z } from "zod";
...
```
Then import and call the registration function in `src/server/tools.ts`.
...
## Adding Resources
Use `createResource()` from `src/lib/factories.ts` in `src/server/resources.ts`:
```ts
...
```
See existing `projects`, `nodes`, and `textures` examples in `src/server/resources.ts`.

## Adding Prompts
Use `createPrompt()` from `src/lib/factories.ts` in `src/server/prompts.ts`:
```ts
import { z } from "zod";
...
```
See `src/server/prompts.ts` for examples using the `readPrompt` macro to embed prompt text files.
...
- The file remains a focused guide for contributor workflow and verification.
- Build, verification, and plugin-load steps are present and actionable.
- Architectural and commit rules match existing repo constraints.
