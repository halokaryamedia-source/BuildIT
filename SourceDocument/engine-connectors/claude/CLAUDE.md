...
**Tool Registration**: Each tool file in `src/server/tools/` follows a two-part pattern:
...
**Prompt Registration**: Use `createPrompt()` from `src/lib/factories.ts` with optional argument completion.

**Resources**: Use `createResource()` from `src/lib/factories.ts` in `src/server/resources.ts`.
...
- 2-space indentation
- Zod for validation; store reusable schemas in `src/lib/zodObjects.ts`
...
- Project architecture and build steps are documented for safe assisted changes.
- Core constraints (no new framework, no unnecessary dependency changes, MCP/tool verification requirements) are explicit.
- A human can start and verify local changes with documented commands and structure.
