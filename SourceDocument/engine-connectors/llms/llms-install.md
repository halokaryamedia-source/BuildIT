...
1. **Is Blockbench running?**
   ...
   - The MCP plugin must be installed (File > Plugins > Load from URL: `https://achmadawdi.github.io/mcp-blockbench/mcp.js`)
...
- Default endpoint (`http://localhost:3000/bb-mcp`) and alternate port cases are clearly documented.
- At least one direct Streamable HTTP path and a `mcp-remote` fallback are present.
- Fallback is explicitly marked as contingency when direct tool listing fails.
- A user can configure a client without prior project memory.
