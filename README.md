...
19. The engine updates `job_snapshot.json`, `artifact_index.json`, and `job_bundle.json` throughout artifact reads.
...
- `GET /api/jobs/:id/artifacts` refreshes `artifact_index.json` and `job_bundle.json`, then returns artifact metadata.
...
Job bundle manifests are stored at `outputs/jobs/<jobId>/job_bundle.json` and describe bundle readiness for future package export.
