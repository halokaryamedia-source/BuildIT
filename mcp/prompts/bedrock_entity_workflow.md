...
Do not inspect each newly placed Cube or capture after every mutation. Reuse fresh returned state. `create_project`/path `export_model` already return lifecycle state; **Do not immediately call `get_project_info`** unless required fields are missing or external state may have changed. Use `inspect_model_bounds` only for envelope/scale/ground/displacement; **Otherwise skip it**. Re-capture only affected reference-corresponding views. `UNVERIFIED` is not a retry command; **Do not spend additional calls trying to remove UNVERIFIED** unless obtainable evidence can change the decision.

## Actual reference grounding

Reference-driven authoring requires the **actual approved reference image visible in active multimodal context**. Filename/path/manifest/text summary/prior observation/memory **is not image evidence**. If the image cannot be inspected, use `BLOCKED`; do not reconstruct form from prose or generic object knowledge.

Authority: user brief/target → identity/function; approved image → visible form; approved dimensions → numeric envelope.
...
Claims cover identity, mass/landmark, count/symmetry, topology/contact, orientation, negative space, representation. No coordinates/pixel calibration/hidden-feature invention belong here.

Create a **View Pair Map** from reference labels to matching canonical `capture_model_views` views. Ambiguous front/back, left/right, mirrored, or 3/4 pairing remains `UNVERIFIED`; do not compare unlike views.
...
A semantic label never authorizes coordinates. Every primary Cube maps to a declared mass/landmark or justified split/relationship; **no orphan/filler Cube**. `PROVISIONAL` may support a coarse non-contradictory hypothesis, but placement never verifies it.
...
`[0,0,0]` is valid only when image evidence supports axis alignment and no **visible material slope** contradicts it. `ROTATED` requires explicit origin/pivot and role `MASS_CENTER | ATTACHMENT | JOINT | PARENT_TRANSFORM`. Material `UNRESOLVED` becomes `BLOCKED`, not silent axis alignment.

For every **required attachment**, identify **contact target/invariant** first. Rotating an attached mass preserves it; use an **attachment/joint pivot** when that owns the transform. Numeric overlap/hierarchy is not contact proof; negative spaces stay open.
...
- `PASS` — fresh paired evidence has no critical/major mismatch for supported claim.

Front PASS is not full 3D PASS when side/depth evidence is missing/fails. Bounds, hierarchy, coordinates, validators, successful mutation, scalar similarity/IoU/projection scores, or fluent review text cannot justify PASS. A material visible slope left axis-aligned is `FAIL` unless intentionally stepped. After material mutation, affected model views are **stale** until re-captured; if approved image is no longer visible, reload it or remain `UNVERIFIED/BLOCKED`.
...
4. `modify_cube` or one coherent `modify_cubes_batch`;
...
`BLOCKED` means continuation requires guessing: approved image unavailable, invalid pairing, unresolved semantic/reference/orientation/contact conflict, unavailable capability/evidence, or repeated failed work.
...
known project → grounded reference → semantic form + primary hypothesis → place_cube / add_group
