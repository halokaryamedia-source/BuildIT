...
function requirePositiveTextureMetric(value: number, context: string): number {
...
export function mapFaceUvToTexturePixels(
  ...
  const uv = requireFiniteInspectableFaceUv(values, context);
  ...
  if (corners.some((value) => !Number.isFinite(value))) {
    throw new Error(
      `${context} produced a non-finite texture-space coordinate and cannot be reported safely.`
    );
  }
