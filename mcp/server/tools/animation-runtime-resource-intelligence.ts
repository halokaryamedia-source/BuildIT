...
function runtimeEvidence(variableDeps:readonly string[]=[]):ClientEntityRuntimeEvidence{
 if(typeof AnimationItem==="undefined")return{variable_dependencies:variableDeps};const all=(AnimationItem.all??[]) as Array<_Animation|AnimationController>,known=all.map(x=>x.name),links:NonNullable<ClientEntityRuntimeEvidence["controller_links"]>=[],effects:NonNullable<ClientEntityRuntimeEvidence["effect_references"]>=[];
