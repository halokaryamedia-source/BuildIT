...
describe("model creation effectiveness — semantic form, rotation, pivot and contact", () => {
  ...
  test("semantic hardening stays in modelling judgement instead of adding self-reported MCP fields", async () => {
    const [cubes, profile] = await Promise.all([source("server/tools/cubes.ts"), source("lib/registrationProfile.ts")]);
    ...
    for (const field of ["semantic_form", "mass_id", "must_exist_reason", "contact_target", "orientation_state", "pivot_role"]) expect(placeContract).not.toContain(field);
