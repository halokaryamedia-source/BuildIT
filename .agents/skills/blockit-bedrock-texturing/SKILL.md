...
`gradient_tool` needs reference-supported continuous transition; no extra discovery/readback.
...
Approved image required. Define stable palette roles `BASE | SHADOW | HIGHLIGHT | ACCENT/IDENTITY` per cohort; keep one hue/value ramp per material, separate palette from lighting. `color_picker_tool` samples atlas, not reference; never loop guess → paint → readback → tiny tweak.
...
Plan material/palette/form/identity before painting. Broad regions → `draw_shape_tool`; connected base → `paint_fill_tool`; same-color disconnected detail → one `paint_with_brush` batch with `connect_strokes=false`.
...
BASE = ownership; VALUE/FORM = form/contact; IDENTITY = mapped marks; SECONDARY DETAIL = controlled only. No noise-first pass.
