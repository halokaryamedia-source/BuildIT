...
Own geometry/form judgement, transform ownership, and reference-grounded modelling continuation.

## Minimum Necessary Evidence

**No per-Cube inspection ceremony** without a diagnosed problem; **No screenshot-per-mutation loop** — build a judgeable whole form, then gate it. Bounds are only for envelope/scale/ground/displacement (**Otherwise skip the bounds call**); `UNVERIFIED` does not automatically require more calls.
...
```text
user brief→identity; approved image→visible form; dimensions→numeric envelope
...
Build a **View Pair Map**; ambiguous front/back, left/right, mirrored, or 3/4 pairing stays `UNVERIFIED`. Fidelity is **Minecraft-first**: recognizability, masses/counts, topology, negative spaces, buildability over exact anatomy.
...
**A semantic label never authorizes coordinates. No orphan Cube, filler Cube.** Every primary Cube implements a declared mass/landmark or justified split.

Choose the **simplest construction that preserves the visible requirement**. Solid Cuboid, plane-like Cube, layered/inflated shell, linked segments, and texture-only are reasoning examples, not presets.
...
### Marketplace-Grade Construction Patterns

- **Thin/plane-like Cubes** for blades, propellers, cloth, fins: a zero-span axis is valid when thickness is not the visible requirement.
- **Inflate layering**: stacked shells/clothing use small deltas (0.1/0.2/0.5) to prevent z-fighting; **negative inflate** is valid for insets/interior shells.
- **Segment chains**: organic curves (tails, spines, blades, limbs) use 2+ Cubes, each in its own Group pivoted at the joint with a small per-link rotation (e.g. `[2,0,0]`) — never unit-Cube staircasing.
- **Mirror pairs**: author one side, then mirror the Cube with `mirror_uv=true`.
- **Locators** own non-visible anchors (hold points, VFX origins) instead of hidden Cubes.
- **Rig depth follows articulation need** (arm0→arm1, foot); depth never justifies bad placement.

## Tool Lane Discipline / Primary Build

**If no current decision requires a branch, stay in the geometry lane.** Normal lane: grounded claims → Semantic Form → transform ownership → Primary Form → coherent Cubes/Groups → necessary views → diagnosed correction.

A **minor reference discrepancy** does not change identity, primary mass/count, topology/attachment, important negative space, buildability, or identity-critical texture information. Resolve with **one canonical interpretation**: user requirement → Source evidence → best-supported view(s) → simplest buildable form. **Do not average drift.** Minor reference discrepancy alone is not a blocker. **A convincing front silhouette does not validate depth.** Only unresolved **material** cross-view conflict → **Enter the workflow `BLOCKED` state**.
...
Search **difference-first** across recognizability, masses/counts, silhouette, orientation, contact, negative spaces. `FAIL` = critical/major mismatch; `UNVERIFIED` = missing/ambiguous/materially conflicting/unavailable evidence; `PASS` = no critical/major supported mismatch. Minor preview drift is not `FAIL` when the canonical interpretation remains recognizable/buildable.

Front PASS is not full 3D PASS when depth evidence fails. Tool success, coordinates, bounds, hierarchy, or **similarity/IoU/projection scores cannot justify PASS**. Mutation makes affected captures stale.
...
**Reuse fresh exact authored state already returned for that target when sufficient**; otherwise `inspect_element` once. Declare target UUID(s), cause, invariant, expected effect. TRANSLATE preserves size; RESIZE names changed axis + fixed anchor/center/contact; ROTATE preserves `from/to/size`, declared pivot role, required attachment. `geometry_effect` must match intent. **An unintended center shift** during center-preserving RESIZE, size change during TRANSLATE, extent change during ROTATE, or broken contact invariant is structurally wrong.
