...
`DIRECT` uses Groups/Cubes. `3D_ASSISTED` keeps its existing pipeline; never auto-switch or let scaffold evidence replace the approved image.

## DIRECT Hot Path
`semantic form → coherent primary batch → one evidence bundle → smallest causal correction`.

For nontrivial form only, make a **transient Primary Mass Contract** before coordinates; reasoning-only, not persisted or an MCP field:
```text
...
First blockout review uses one `capture_model_views` call with **Core View Triad** `front + left + top`: width×height, length×height, width×length. Add `back` only for rear topology/asymmetry; add `front_left_3q` only for attachment/layering/orientation ambiguity. Never recapture all five routinely.

For repeated/symmetric cohorts, derive absolute transforms once → one coherent `manage_cubes` batch. Preserve supported asymmetry; never loop inspect→write per Cube.
...
```
A semantic label never authorizes coordinates. **No orphan/filler Cube**. `PROVISIONAL` is a coarse hypothesis; placement never verifies it. Construction forms are **not presets**. Surface-only detail stays texture.
...
Depth: `OBSERVED` direct; `INFERRED` consistent evidence; `UNRESOLVED` insufficient/conflicting. Do not average drift. Only unresolved material conflict becomes `BLOCKED`.
...
Stay in Geometry unless another owner is required:
