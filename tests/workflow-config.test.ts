...
describe("workflow configuration", () => {
  ...
  test("connection uses one fixed endpoint", () => {
    ...
    expect(profile.tool_profile_contract).toBe("engines/shared/profiles/tool-profiles.json");
  });

  test("four user-visible stage profiles remain singular", () => {
    const stages = readJson("engines/shared/profiles/stage-profiles.json").profiles;
    expect(Object.keys(stages).sort()).toEqual(["ANIMATION", "FINAL_VALIDATION", "GEOMETRY", "TEXTURE"].sort());
  });
});
